...
def bn_conv_relu(input, filters, bachnorm_momentum, **conv2d_args):
    x = layers.BatchNormalization(momentum=bachnorm_momentum)(input)
    x = layers.Conv2D(filters, **conv2d_args)(x)
    return x


def bn_upconv_relu(input, filters, bachnorm_momentum, **conv2d_trans_args):
    x = layers.BatchNormalization(momentum=bachnorm_momentum)(input)
    x = layers.Conv2DTranspose(filters, **conv2d_trans_args)(x)
    return x
...
def satellite_unet(
    ...
    outputs = layers.Conv2D(num_classes, kernel_size=(1, 1), strides=(1, 1), activation=output_activation, padding='valid')(x)
