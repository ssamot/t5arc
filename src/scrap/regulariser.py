...
class SVDLinearRegression:
    ...
    def fit(self, X, y):
        ...
        XTX_reg = ops.dot(X_b.T, X_b) + epsilon * ops.eye(X_b.shape[1])
        #print(XTX.shape)
        XTy = ops.dot(X_b.T, y)
        #print(XTX_reg.shape, XTy.shape, "shapes")
        # exit()
        # theta = keras.ops.solve(X, y.T)

        theta = ops.dot(ops.linalg.inv(XTX_reg), XTy)
        #theta = ops.linalg.solve(XTX_reg, XTy)
        #print(theta)
        #exit()
